...
function FolderControlButton(props: IconButton){
    const { icon, active, onClick } = props;
    
    function renderIcon(): any {
        if(icon === "trash"){
            return <TrashIcon size={26} fill={active === false ? "#000" : "#fff"} />;
        } else if(icon === "settings"){
            return <SettingsIcon size={26} fill={active === false ? "#000" : "#fff"} />;
        } else if(icon === "open_browser"){
            return <OpenBrowserIcon size={26} fill={active === false ? "#000" : "#fff"} />;
        } else if(icon === "collapse_expand"){
            if(active === true){
                return <CollapseIcon size={26} fill={"#fff"} />;
            } else if(active === false){
                return <ExpandIcon size={26} fill={"#000"} />;
            }
        }
    }
